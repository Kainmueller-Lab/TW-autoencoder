...
def heatmap_plot(model, test_loader, dict,device, iteration, n_masks, save_folder):
    ...
    for batch_idx, (data, class_labels, sem_gts, sem_gt_exists) in tqdm(enumerate(test_loader)):
        ...
        if batch_idx in img_idx_list:
            ...
            img_name=f"{dict['name']}_{batch_idx}_iter_{iteration}.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_folder', type=str, default='../snapshot/') 
    ...
    st_unet_dict={
        'name':'std_unet',
        'list_n_masks':[100,500,None],
        'list_iter':[252000,259500,304500]
    ...
    save_folder=f"../results/heatmaps/{st_unet_dict['name']}_{args.backbone}_lr{args.lr}_bs{args.batch_size}"

    ###############generate_figures###############
    if args.function== 'generate':
        model=intialize_model(args, st_unet_dict)
        ...
        with torch.no_grad():
            ...
            for n_masks, iteration in zip(st_unet_dict['list_n_masks'],st_unet_dict['list_iter']):
                    model=load_checkpoint(args, model,st_unet_dict['name'], iteration, n_masks)
                    heatmap_plot(model, test_loader, st_unet_dict,device, iteration, n_masks,save_folder)
    else:
