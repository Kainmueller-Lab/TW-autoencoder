...
class VGG_CBB_Decoder(nn.Module):
    def __init__(self, encoder, backbone, no_skip_connection):
        super(VGG_CBB_Decoder, self).__init__()
...
class Resnet_CBB_Decoder(nn.Module):
    def __init__(self, encoder, backbone, no_skip_connection):
        super(Resnet_CBB_Decoder, self).__init__()
...
class Inv_BasicBlock(nn.Module):
    def __init__(self,i, block, skip_channels,outpadding):
        ...
        if block.downsample is not None:
        ...
        elif block.downsample is None and skip_channels!=0:
            ...
            # random define self.downsample which is just used for aligning the output channels
            self.downsample=nn.Sequential(
                nn.Conv2d(in_channels=block.conv2.out_channels+skip_channels, out_channels=block.conv1.in_channels,
                                        kernel_size=3, stride=block.conv2.stride, padding=block.conv2.padding),
                nn.BatchNorm2d(block.conv1.in_channels)
            )
        else:
...
class Inv_Bottleneck(nn.Module):
    def __init__(self, i, block, skip_channels,outpadding):
        ...
        if block.downsample is not None:
            ...
            # random define self.downsample which is just used for aligning the output channels
            self.downsample=nn.Sequential(
                nn.Conv2d(in_channels=block.conv3.out_channels+skip_channels, out_channels=block.conv1.in_channels,
                                        kernel_size=1, stride=1),
                nn.BatchNorm2d(block.conv1.in_channels)
            )
        else:
