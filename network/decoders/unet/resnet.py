...
class Resnet_Decoder(nn.Module):
    ...
    def forward(self, *features):
        ...
        x = torch.cat([self.inv_maxpool(x), features[-5]], dim=1)
    ...
    def _make_inv_layer(
            self, layer,skip_channels, outpadding, bottle_up=False):
        ...
        if bottle_up:
            if isinstance(layer[0], BasicBlock):
                inv_layer.append(
                    Inv_BasicBlock(
                        layer[0], skip_channels, outpadding))
